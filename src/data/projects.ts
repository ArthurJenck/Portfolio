...
export const projectsArr = [
    ...
    },
    {
        ...
        desc: "Découverte de React lors du 5ème projet réalisé dans mon cursus OpenClassrooms, en intégrant un site de location d'appartement entre particuliers. Héberger une application React sur GitHub Pages s'avéra plus compliqué que prévu, la plateforme ne pouvant lancer de script du package.json. Après de nombreuses recherches et la création d'un fichier deploy.yml, j'ai finalement atteint un hébergement GitHub Pages fonctionnel.\nInitialiser une application React avec CRA et Vite • Routage avec react-router-dom • Structuration en composants réutilisables • Stylisation en SCSS • Typage strict avec TypeScript • Animations CSS • Hébergement via GitHub Actions",
    ...
    },
    {
        ...
        desc: "Au cours de mon apprentissage de JavaScript pendant ma 2ème année à l'ECV Paris, j'ai été amené à créer un jeu de Morpion. Ma méthode de création s'est basée sur les classes CSS, ainsi qu'un tableau contenant toutes les conditions de victoire. Le plus dur fut la définition des conditions de victoire, pour lesquelles j'ai dû faire un tableau regroupant chaque combinaison gagnante possible.\nCréation d'un jeu en JavaScript • Gestion des conditions de victoire • Prendre en compte les égalités • Stylisation en SCSS",
    ...
    },
    {
        ...
        picMobile: bluelMob,
        techs: [techsArr[0], techsArr[2]],
        desc: "3ème projet de mon parcours OpenClassrooms, consistant à intégrer une page de connexion ainsi qu'une modale sur la page d'admin du site d'une designeuse. Une API fournie dans la base de code est liée aux projets, et stockée dans le localStorage. Il est toujours compliqué de s'y retrouver dans un code sur lequel on vient de passer plusieurs heures. Une structuration claire de mes fichiers de scripts fut salvatrice.\nCréation d'une page dynamique en Vanilla JS • Utiliser des formulaires pour récupérer des données utilisateur • Manipuler les éléments du DOM • Gérer les événements et leurs écouteurs • Stocker des données dans le localStorage • Communiquer dynamiquement avec l'API • Authentification d'administrateur • Implémenter le CRUD",
